...
* jQuery atd js
*
* http://plugins.jquery.com/project/AtD
*
* Depends:
*	jquery.min.js
*/
...
AtD._checkTextArea = function (id, commChannel, linkId, after) {
    ...
    var options = AtD.textareas[id]; if (options['link'].html() != options['before']) { AtD.restoreTextArea(id); }
        ...
        }; div.keypress(syncContents); div.mousemove(syncContents); div.mouseout(syncContents); div.css(options['style']); div.height(options['height']); commChannel(id, { ready: function (errorCount) { options['link'].unbind('click', disableClick); }, explain: function (url) { var left = (screen.width / 2) - (480 / 2); var top = (screen.height / 2) - (380 / 2); window.open(url, '', 'width=480,height=380,toolbar=0,status=0,resizable=0,location=0,menuBar=0,left=' + left + ',top=' + top).focus(); }, success: function (errorCount) {
            if (errorCount == 0)
